...
def carTransfer(request):
    car_transfer = Owner(fullname=request.data['name'], national_id=request.data['national'],
    ...
    if car_transfer:
        ...
        print(car_r)
        response2 = {'name': car_transfer.fullname, 'national': car_transfer.national_id, 'mobile': car_transfer.mobile,
                     'dob': car_transfer.dob,
                     'pin': car_transfer.pin, 'email': car_transfer.email, 'reg_no': car_transfer.reg_id,
                     'type': car_transfer.vehicle_type,
                     'make': car_transfer.make, 'model': car_transfer.vehicle_model,
...
# Launch Page
def carOwner(request, user):
    vehicles = Owner.objects.filter(national_id=user)
    return render(request, 'car_reg/vehicle_transfer_owner.html', {'vehicles': vehicles})


# Car Owner View
def carOwnerView(request, vehicle):
    vehicle = Owner.objects.get(owner_id=vehicle)
    return render(request, 'car_reg/vehicle_transfer_owner_view.html', {'vehicle': vehicle})
